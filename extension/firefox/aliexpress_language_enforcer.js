...
// Force any AliExpress locale (like xx.aliexpress.com) page to redirect to its global equivalent and saves location/currency settings.
//Please clear cookies on all "aliexpress" domains before using this userscript.
// https://github.com/agentzex/aliexpress_force_global



const FORCE_SITE = 'glo';
let FORCE_REGION = 'US';
let FORCE_LOCALE = 'en_US';
let FORCE_CURRENCY = 'USD';


// Load saved settings using the browser API
async function loadSettings() {
	try {
		const result = await browser.storage.local.get(['FORCE_REGION', 'FORCE_CURRENCY']);
		console.log("Loaded settings:", {
			result
		});

		// Update global variables with the retrieved values or keep defaults
		FORCE_REGION = result.FORCE_REGION || FORCE_REGION;
		FORCE_CURRENCY = result.FORCE_CURRENCY || FORCE_CURRENCY;
